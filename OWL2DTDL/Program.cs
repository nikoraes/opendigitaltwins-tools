    ...
    private static void GenerateInterfaces()
    {
      ...
      foreach (OntologyClass oClass in _ontologyGraph.OwlClasses.Where(oClass => oClass.IsNamed() && !oClass.IsDeprecated() && !IsIgnored(oClass) && !oClass.SuperClasses.Any(parent => parent.IsNamed() && IsIgnored(parent))))
      {
        ...
        !IsIgnored(relationship.Property)))
        {
          ...
          // If this relationship is annotated with the dtdlType annotation
          // and that type is an enumeration, then assert the relationship is an enumeration property
          if (!relationship.Target.IsOwlThing() && relationship.Target.IsNamed() && relationship.Target.DtdlTypes().Contains("enum"))
          {
            ...
            ILiteralNode propertyNameNode = dtdlModel.CreateLiteralNode(relationshipName);
            ...
            dtdlModel.Assert(new Triple(enumNode, rdfType, dtdl_Enum));
            dtdlModel.Assert(new Triple(propertyNode, dtdl_schema, enumNode));

            IEnumerable<Individual> enumOptions = _ontologyGraph.GetIndividuals(relationship.Target);
            // Always string as schema
            dtdlModel.Assert(new Triple(enumNode, dtdl_valueSchema, dtdl_string));
            foreach (Individual option in enumOptions)
            {
              // Only include relationships with valid names
              string enumOptionName = string.Concat(option.GetLocalName().Take(64));
              if (!IsCompliantDtdlName(enumOptionName))
              {
                Console.Error.WriteLine($"Unable to add enum option '{enumOptionName}' on property '{relationshipName}'; underlying name does not adhere to DTDL regex.");
                continue;
              }

              IBlankNode enumOption = dtdlModel.CreateBlankNode();
              // TODO also add displayName and description
              dtdlModel.Assert(new Triple(enumOption, dtdl_name, dtdlModel.CreateLiteralNode(enumOptionName)));
              dtdlModel.Assert(new Triple(enumOption, dtdl_enumValue, dtdlModel.CreateLiteralNode(enumOptionName)));

              // If there are rdfs:labels, use them for DTDL displayName
              if (option.Label.Any())
              {
                dtdlModel.Assert(GetDtdlDisplayNameTriples(option, enumOption));
              }

              // If there are rdfs:comments, generate and assert DTDL description triples from them
              if (option.Comment.Any())
              {
                dtdlModel.Assert(GetDtdlDescriptionTriples(option, enumOption));
              }

              dtdlModel.Assert(new Triple(enumNode, dtdl_enumValues, enumOption));
